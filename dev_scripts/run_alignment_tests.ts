...
import {PlaneWordMap, CatBoostWordMap, MorphCatBoostWordMap, FirstLetterBoostWordMap, JLBoostWordMap, JLBoostMultiWordMap } from "../src/boostwordmap_tools";
...
function run_configurable_wordmap_test( alignment_adding_method: number, boost_type: string, lang_selections: string, ratio_of_training_data: number ){
    const boostMap = (boost_type === "jlboostmwm" )?  new JLBoostMultiWordMap    ({ targetNgramLength: 5, warnings: false }):
                     (boost_type === "jlboost"    )?  new JLBoostWordMap         ({ targetNgramLength: 5, warnings: false }):
...
if (require.main === module) {
    ...
    //run_configurable_wordmap_test( 2, "jlboostmwm", "heb-english-gen", 1 )
